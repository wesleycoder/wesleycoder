...
export const loop = () => {
  for (const spawn of Object.values(Game.spawns)) {
    const currentHarvesters = Object.values(Game.creeps).filter((creep) => creep.memory.role === 'harvester').length
    if (spawn.store.energy > basicHarvesterCost) {
      if (targetHarvesters > currentHarvesters) {
        if (spawn.spawnCreep(basicHarvesterBody, `harvester${Game.time}`, { memory: { role: 'harvester' } }) === OK) {
          ...
          console.log('failed to spawn harvester')
        }
      }
    }
  }
