...
BOOL swizzled_canBecomeKeyWindow(id self, SEL _cmd) {
  if ([self isKindOfClass:[NSWindow class]]) {
    ...
    if ((mask & NSWindowStyleMaskTitled) == 0) return YES;
  }
...
BOOL swizzled_canBecomeMainWindow(id self, SEL _cmd) {
  if ([self isKindOfClass:[NSWindow class]] && ([(NSWindow *)self styleMask] & NSWindowStyleMaskBorderless)) {
    return YES;
  }
...
void apply_style_internal(void* handle, float r, float g, float b, float a, int vibrant) {
  ...
  dispatch_once(&onceToken, ^{
    Method m1 = class_getInstanceMethod([win class], @selector(canBecomeKeyWindow));
    original_canBecomeKeyWindow = method_setImplementation(m1, (IMP)swizzled_canBecomeKeyWindow);

    Method m2 = class_getInstanceMethod([win class], @selector(canBecomeMainWindow));
    original_canBecomeMainWindow = method_setImplementation(m2, (IMP)swizzled_canBecomeMainWindow);
  });
