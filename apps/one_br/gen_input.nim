#!/usr/bin/env nim r
import std/[parseopt, random, strformat, strutils, os]
...
proc printUsage() =
  ...
  echo "Options:"
  echo "  -l, --line-order    Order of multiplication for line count (2=100, 3=1000, etc)"
  echo "  -h, --help          Show this help message"

proc parseArgs(): tuple[fileName: string, order: int] =
  var
    fileName = ""
    order = 0
    args = initOptParser()
...
proc main() =
  let (outputName, order) = parseArgs()
  let targetLines = fmt"1e{order}".parseFloat.toInt
  echo fmt"Creating `{outputName}` with {targetLines} lines"

  var linesWritten = 0
  var bytesWritten = 0.0

  let dir = splitFile(outputName).dir
  if dir != "":
    createDir(dir)
  ...
  echo fmt"Done: {bytesWritten.int64.formatSize('.', bpColloquial)} in size"
